	...
	for (const name of Object.keys(ENV.bundles)) {
		...
		streams.push(
			sass(`${PATH.dir.cacheStyles}/${name}/collections/*.${PATH.ext.styles}`, {
				...
				.pipe(gulpif(bundle.styles.options.minify && !ENV.watching, cssnano({  autoprefixer:{ browsers:bundle.styles.options.autoprefixer }, reduceIdents:false, zindex:false })))
